...
public abstract class FireNodeBase : MonoBehaviour, IFireWaterTarget
{
    ...
    [Header("Runtime Debug Visual")]
    public bool hideDebugVisualOnPlay = true;
    public Transform debugVisualRoot;
    ...
    private void HideDebugVisualIfNeeded()
    {
        if (!hideDebugVisualOnPlay)
        {
            return;
        }

        Transform root = debugVisualRoot != null ? debugVisualRoot : transform.Find("Visual");
        if (root == null)
        {
            return;
        }
